...
function _Initheatmap(h = -1) {
    try {
        ...
        heatmap = h337.create({
            ...
            radius: s,
            gradient: {
                0: "rgba(0, 0, 0, 1)",  // Start with black
                1: "rgba(0, 0, 0, 1)"   // End with black
            });
        heatmap.addData = function (dataPoint) {
            // Directly draw a black circle instead of relying on a gradient
            var ctx = this._renderer.shadowCtx;
            ctx.beginPath();
            ctx.arc(dataPoint.x, dataPoint.y, dataPoint.radius, 0, Math.PI * 2); ctx.fillStyle = "rgba(0, 0, 0, 1)";  // Force black color
            ctx.fill();
        };
